...
--       print $ search (Many (Lit A :> Lit B)) [B,A,B] == Just [A, B]
       ...
       writeln "accept sum of lanugaguages when one is accepted"
       quickCheck acceptsSum

       writeln "simple findall test"
       print $ findall number "heh to jest ważny 146846 numer w tym napisie 78 xd" == ["146846","78"]

       writeln "findall Benke test"
       print $ findall (Many (Lit 'a' :> Lit 'b')) "aabab" == ["","abab"]

       writeln "findall evil Benke test"
       print $ findall ((Lit 'a' :> Lit 'b') :| (Lit 'b' :> Lit 'c')) "abc" == ["ab", "bc"]

       writeln "simpl reducing test"
       print $ 4 == size (simpl $ ders (replicate 1000 A) (Many (Lit A) :> Lit B))

       writeln "mayStart x so can Many(x)"
       quickCheck manyMayStart

       writeln "simple Benke search test"
       print $ search number "It is important to note that uid 0 is special in Unix" == Just "0"

       writeln "Accepting word implies language may start with it beginning"
       quickCheck acceptImpliesMayStart
...
acceptImpliesMayStart :: [AB] -> Reg AB -> Property
acceptImpliesMayStart c r = not (null c) && accepts r c ==> mayStart (head c) r

mayStartConcat :: AB -> Reg AB -> Reg AB -> Property
mayStartConcat c x y = mayStart c x ==> mayStart c (x:>y)

manyMayStart :: AB -> Reg AB -> Property
manyMayStart c x = mayStart c x ==> mayStart c $ Many x
